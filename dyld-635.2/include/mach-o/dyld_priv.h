...
// initializers in that image.  This is when objc calls any +load methods in that image.
    ...
    /*
     注意：只在objc运行时使用。当调用objc映像映射、未映射和初始化时，注册处理程序。Dyld将使用包含Objc镜像信息部分的一系列镜像调用“映射”函数。那些是Dylibs的镜像将自动重新计算，因此Objc将需要调用dlopen（）来防止它们被卸载。在通话期间 _dyld_objc_notify_charter(), Dyld将调用加载的Objc映像调用“映射”函数。在随后的dlopen（）调用中，Dyld也将调用“映射”函数。当DYLD在镜像中被称为初始化器时，Dyld将调用“init”函数。这是objc在镜像中调用任何+load方法的时候。
     */
//
