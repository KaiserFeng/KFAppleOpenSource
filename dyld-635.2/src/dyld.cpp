...
{
	// try mach-o loader
	//首先检测MachO的头部信息magic、cputype、cpusubtype等相关属性，判断Mach-O文件的兼容性，满足条件 继续执行
	if ( isCompatibleMachO((const uint8_t*)mh, path) ) {
		//创建ImageLoader，负责将MachO实例化为Image镜像，并添加到全局镜像列表 sAllImages，并将image映射到申请的内存中
...
#endif
	//如果设置了 DYLD_PRINT_OPTS， 则调用 printOptions 打印参数
	if ( sEnv.DYLD_PRINT_OPTS )
		printOptions(argv);
	//如果设置了 DYLD_PRINT_ENV， 则调用 printEnvironmentVariables 打印参数
...
reloadAllImages:
		...
		instantiateFromLCRSetCrashLogMessage(sLoadingCrashMessage);
		// instantiate ImageLoader for main executable
		//3、实例化主程序 上面还有一堆操作不知道干嘛的 哈哈
		...
		if ( (sAllCacheImagesProxy != NULL) && ImageLoader::haveInterposingTuples() ) {
			...
			//我们不需要担心插入的镜像，因为如果设置了DYLD_INSERT_LIBRARIES，我们就不会使用加速器表
			sAllImages.clear();
			sImageRoots.clear();
			sImageFilesNeedingTermination.clear();
		...
		//8、执行初始化方法
	 //dyld会优先初始化动态库，然后初始化主程序。该函数首先执行runInitializers()，内部再依次调用processInitializers()、recursiveInitialization()。我们在recursiveInitialization()函数里找到了notifySingle()函数,notifySingle()函数回调通知Objc的runtime
		initializeMainExecutable(); 
	#endif
